...
router.post('/changepass1', checkAuth, async (req, res) => {
    const userID = req.user.ID;
    const password = req.body.password;

    const findAccount = await database.simpleExecute(`SELECT * FROM OAUTH.USERS WHERE USERS.ID=${userID}`);
    if(findAccount.rows.length === 0){
        res.status(401).json({
            statusCode: 0,
...
router.patch('/changepass2', checkAuth, async (req, res) => {
    const userID = req.user.ID;
    const password = req.body.password;
    const hash = bcrypt.hashSync(password, parseInt(process.env.SALT_ROUNDS));

    const updatePass = await database.simpleExecute(`UPDATE OAUTH.USERS SET USERS.PASSWORD='${hash}' WHERE USERS.ID=${userID}`);
    
    res.status(401).json({
        statusCode: 1,
        message: "successfully updated pass"
    })
});

//upload image
router.post('/profilepicture', checkAuth, upload.single('productImage'), async (req, res) => {
    const user = req.user;
    let img_url;
    ...
    const result = await database.simpleExecute(`UPDATE EXOADM.USUARIOS SET IMG_URL='${img_url}' WHERE COD_USUARIO=${user.ID}`);
    res.status(200).json({
        statusCode: 1,
        result: result
    })
...
router.patch('/forgotpassword', async (req, res) => {
    ...
    if(findAccount.rows.length === 0){
        res.status(401).json({
            statusCode: 0,
            message: "Failed email not registered"
        })
    }
    else{
        ...
        const updatePass = await database.simpleExecute(`UPDATE OAUTH.USERS SET USERS.PASSWORD='${hash}' WHERE USERS.EMAIL='${email}'`);
        const emailmessage = {
            from: '"EXO|BUY" <api@myexobuy.com>', // Sender address
            to: email,         // List of recipients
            subject: 'Reseteo de Contrasena', // Subject line
            text: 'Tu nueva contrasena temporal para logiar\n'+'password: ' + password
        };
        transporter.sendMail(emailmessage, function(err, info) {
            if (err) {
                console.log(err);
