...
router.post('/mercados', checkAdmin, async (req, res) => {
    ...
    const limite_maximo = req.body.limite_maximo;
    const result = await database.simpleExecute(`INSERT INTO EXOADM.MERCADOS (MERCADOS.MERCADO, MERCADOS.LIMITE_MAXIMO) VALUES('${mercado}', ${limite_maximo})`);
    res.status(200).json({
        statusCode: 1,
        result: result
    })
...
router.post('/create', checkVendor, upload.single('productImage'), async (req, res) => {
    ...
    const result = await database.simpleExecute(`INSERT INTO EXOSTO.TIENDAS (TIENDAS.TIENDA, TIENDAS.COD_USUARIO, TIENDAS.FECHA_INICIO_TIENDA, TIENDAS.COD_MERCADO, TIENDAS.DIRECCION_FISICA, TIENDAS.TIENDA_DETALLE, TIENDAS.NUM_TELF, TIENDAS.IMG_URL) VALUES('${nombre_tienda}', ${user.ID}, CURRENT_DATE, ${cod_mercado}, '${direccion}', '${detalle}', '${num_telf}', '${img_url}')`);
    res.status(200).json({
        statusCode: 1,
        result: result
    })
...
router.post('/categories', checkVendor, async (req, res) => {
    ...
    const result = await database.simpleExecute(`INSERT INTO EXOSTO.CATEGORIAS_PRODUCTOS (CATEGORIAS_PRODUCTOS.CATEGORIA_PRODUCTO, CATEGORIAS_PRODUCTOS.COD_TIENDA) VALUES('${category}', ${cod_tienda})`);
    res.status(200).json({
        statusCode: 1,
        result: result
    })
...
router.post('/products', checkVendor, upload.single('productImage'), async (req, res) => {
    ...
    const result = await database.simpleExecute(`INSERT INTO EXOSTO.PRODUCTOS (PRODUCTOS.PRODUCTO, PRODUCTOS.COD_TIENDA, PRODUCTOS.COSTO_PRODUCTO, PRODUCTOS.PESO_PRODUCTO, PRODUCTOS.CANTIDAD_ACTUAL, PRODUCTOS.COD_CATEGORIA_PRODUCTO, PRODUCTOS.COD_BARRA, PRODUCTOS.COD_TIPO_UNIDAD, PRODUCTOS.PRODUCTO_DETALLE, PRODUCTOS.IMG_URL) VALUES('${producto}', ${cod_tienda}, ${costo_producto}, ${peso_producto}, ${cantidad}, ${cod_categoria_producto}, '${cod_barra}', ${cod_tipo_unidad}, '${detalle}', '${img_url}')`);
    res.status(200).json({
        statusCode: 1,
        result: result
    })
...
router.get('/vendor', checkVendor, async (req, res) => {
    const user = req.user;
    const result = await database.simpleExecute(`SELECT * FROM EXOSTO.TIENDAS WHERE TIENDAS.COD_USUARIO=${user.ID} ORDER BY TIENDAS.COD_MERCADO, TIENDAS.TIENDA`);
    res.status(200).json({
        data: result.rows
    })
});
...
router.patch('/vendor', checkVendor, async (req, res) => {
    ...
    const result = await database.simpleExecute(`UPDATE EXOSTO.TIENDAS SET TIENDAS.TIENDA='${tienda}', TIENDAS.TIENDA_DETALLE='${tienda_detalle}', TIENDAS.DIRECCION_FISICA='${direccion}', TIENDAS.NUM_TELF='${num_telf}' WHERE TIENDAS.COD_USUARIO=${user.ID} AND TIENDAS.COD_TIENDA=${cod_tienda}`);
    res.status(200).json({
        statusCode: 1,
        result: result
    })
